...
void check_arguments(int num_comd_line_arg,
...
void process_files(int num_comd_line_arg, char **array_strings_comd, Opt fl,
                   SearchSettings *ExamplSearchSettings, char *pattern) {
...
int main(int num_comd_line_arg, char *array_strings_comd[]) {
  ...
  if (!ExamplSearchSettings.bug) {
    verify_options(num_comd_line_arg, array_strings_comd, &fl,
                   &ExamplSearchSettings, &pattern);
  }

  if (!ExamplSearchSettings.bug) {
    handle_pattern(array_strings_comd, &fl, &pattern);
    process_files(num_comd_line_arg, array_strings_comd, fl,
                  &ExamplSearchSettings, pattern);
  } else {
...
void initialize_flags(Opt *fl) { *fl = (Opt){false}; }
...
void process_flag_f(Opt *fl, SearchSettings *settings, char **pattern,
                    char *optarg) {
  fl->f = true;
...
void check_for_errors(int num_comd_line_arg, char *array_strings_comd[],
                      SearchSettings *settings) {
  if (optind >= num_comd_line_arg || array_strings_comd[optind][0] == '-' ||
      strlen(array_strings_comd[optind]) == 0) {
    settings->bug = true;
  }
}

void verify_options(int num_comd_line_arg, char *array_strings_comd[], Opt *fl,
                    SearchSettings *settings, char **pattern) {
  initialize_flags(fl);

  int option;
  while ((option = getopt(num_comd_line_arg, array_strings_comd,
                          "e:ivclnhsf:o")) != -1 &&
         !(settings->bug)) {
...
void read_patterns_from_file(SearchSettings *ExamplSearchSettings,
...
void search_in_file(Opt fl, SearchSettings *ExamplSearchSettings,
...
void regex_compile(Opt fl, SearchSettings *ExamplSearchSettings, regex_t *regex,
                   char *pattern) {
...
void process_flag_o(Opt fl, SearchSettings *ExamplSearchSettings, FILE *file,
...
void process_file_lines(Opt fl, SearchSettings *ExamplSearchSettings,
