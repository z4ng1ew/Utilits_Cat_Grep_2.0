...
int main(int num_comd_line_arg, char* array_strings_comd[]) {
  Opt short_opt = {false};
  bool error_opt =
      verify_options(num_comd_line_arg, array_strings_comd, &short_opt);
...
bool verify_options(int num_comd_line_arg, char* array_strings_comd[],
                    Opt* short_opt) {
  bool error_opt = true;
  int option, option_index = 0;

  while ((option = getopt_long(num_comd_line_arg, array_strings_comd,
                               "beEnstTv", long_options, &option_index)) !=
             -1 &&
         error_opt) {
    switch (option) {
      case 'b':
  ...
  if (!error_opt) {
    fprintf(stderr,
            "Возможно использование только следующих опций: -b -e -E -v -n -s "
            "-t -T\n");
  }
